...
class WireguardTunnel {
  ...
  Future<void> startTunnel() async {
    if (_confFilePath == null && _confString != null) {
      ...
      final tempFile = File(
        '${tempDir.path}${Platform.pathSeparator}mikrotik_wg.conf',
      );
    ...
    if (!await confFile.exists()) {
      throw FileSystemException(
        'WireGuard configuration file not found.',
        _confFilePath!,
      );
    }
    ...
    try {
      if (Platform.isWindows) {
        ...
        var result = await Process.run(wireguardExe, [
          '/installtunnelservice',
          confFile.absolute.path,
        ]);
          ...
          if (stderrLower.contains('access is denied') ||
            ...
            if (silent) {
              throw ProcessException(
                wireguardExe,
                ['/installtunnelservice'],
                'Administrator privileges required to start WireGuard tunnel. Run application as Administrator.',
                result.exitCode,
              );
            }
            _print(
              'WireGuard requires Administrator privileges to start. Requesting elevation...',
            );
            ...
            if (result.exitCode != 0) {
              throw ProcessException(
                'powershell',
                ['Start-Process'],
                'Failed to elevate and start WireGuard tunnel.',
                result.exitCode,
              );
            }
          } else {
        ...
        // macOS and Linux
        var result = await Process.run('wg-quick', [
          'up',
          confFile.absolute.path,
        ]);
        final stderrLower = result.stderr.toString().toLowerCase();
          ...
          if (stderrLower.contains('permission denied') ||
            ...
            if (silent) {
              throw ProcessException(
                'wg-quick',
                ['up'],
                'Root privileges required to start WireGuard tunnel. Run application with sudo/Admin.',
                result.exitCode,
              );
            }
            ...
            if (Platform.isMacOS) {
              ...
              result = await Process.run('osascript', [
                '-e',
                'do shell script "wg-quick up \\"${confFile.absolute.path}\\"" with administrator privileges',
              ]);
              ...
              if (await _isCommandAvailable('pkexec')) {
                ...
                throw StateError(
                  'Permission denied starting wg-quick. Please run this script with sudo.',
                );
              }
            }
  ...
  Future<void> stopTunnel() async {
    ...
    try {
      if (Platform.isWindows) {
        ...
        if (result.exitCode != 0 && !stderrLower.contains('does not exist')) {
          if (stderrLower.contains('access is denied') ||
            ...
            _print(
              'Warning: Failed to stop WireGuard tunnel: ${result.stderr}',
            );
          }
        }
      } else {
          ...
          if (stderrLower.contains('permission denied') ||
            ...
            if (!silent) {
              if (Platform.isMacOS) {
                ...
                result = await Process.run('osascript', [
                  '-e',
                  'do shell script "wg-quick down \\"${_confFilePath!}\\"" with administrator privileges',
                ]);
              } else if (Platform.isLinux) {
                if (await _isCommandAvailable('pkexec')) {
                  _print(
                  ...
                  _print(
                    'Warning: Permission denied stopping wg-quick. Please run with sudo.',
                  );
                }
              }
            }
      ...
      if (e.errorCode == 2 ||
          ...
          e.message.toLowerCase().contains('no such file')) {
        _print(
          'Warning: WireGuard CLI not found. Unable to automatically close tunnel.',
        );
      }
    }
  ...
  Future<void> _installWireGuardOS() async {
    if (Platform.isWindows) {
      ...
      if (response.statusCode != 200) {
        throw StateError(
          'Failed to download WireGuard installer. HTTP ${response.statusCode}',
        );
      }

      final tempDir = Directory.systemTemp;
      final installerFile = File(
        '${tempDir.path}${Platform.pathSeparator}wireguard-installer.exe',
      );
      ...
      if (result.exitCode != 0) {
        throw ProcessException(
          'WireGuard Installer',
          ['/S'],
          'Failed to install WireGuard. If silent=true, ensure you are running as Administrator.\n${result.stderr}',
          result.exitCode,
        );
      }
      ...
      throw StateError(
        'WireGuard auto-installation is not supported on this OS.',
      );
    }
  }
